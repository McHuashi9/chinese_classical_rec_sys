...
- **文言文库数据包更靠得住**：内容库重建改为**整体单事务**，且先写临时文件、成功后再原子替换——重建中途失败时，手机里的旧数据包保持原样（以前可能留下「新文章 + 旧题库」的坏库，界面不报错但答题会对不上文章）。发布前的自动校验也新增了外键完整性检查，坏包不会再被放出去。

> 本版本同时完成了 C++ 核心的内部重构（数据库访问改为 RAII 封装、桥接层按功能分域拆分、题目读取逻辑去重），**不改变任何功能表现、不改动跨语言接口（FFI）契约**，普通用户无感知。
