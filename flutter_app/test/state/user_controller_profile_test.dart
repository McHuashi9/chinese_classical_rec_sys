...
void main() {
  group('UserController 档案状态', () {
    ...
    test('refreshProfiles 按最近使用倒序排列，同时间按 id 升序', () {
      repo.profiles
        ..clear()
        ..add(const UserProfile(
            id: 2, name: '小明', createdAt: 1002, lastUsedAt: 300))
        ..add(const UserProfile(
            id: 3, name: '小红', createdAt: 1003, lastUsedAt: 200))
        ..add(const UserProfile(
            id: 4, name: '同时间A', createdAt: 1004, lastUsedAt: 150))
        ..add(const UserProfile(
            id: 5, name: '同时间B', createdAt: 1005, lastUsedAt: 150))
        ..add(const UserProfile(
            id: 1, name: '默认用户', createdAt: 1001, lastUsedAt: 100));
      repo.activeId = 1;
      ctrl.refreshProfiles();
      expect(ctrl.profiles.map((p) => p.id), [2, 3, 4, 5, 1]);
    });
    ...
    test('createProfile 规范化名称并刷新列表', () {
      ...
      expect(ctrl.profiles.first.name, '小明');
    });
    ...
    test('createInheritedProfile 创建继承档案并刷新列表', () {
      ...
      expect(id, 2);
      expect(ctrl.profiles.length, 2);
      expect(ctrl.profiles.first.name, '继承者');
    });
