...
TEST_CASE("多用户 - 删除档案后学习数据彻底清除", "[multi_user]") {
    db_close();
    const std::string content = test_helpers::makeContentDb("mu_hard_delete");
    const std::string user = test_helpers::makeUserDb("mu_hard_delete");
    REQUIRE(db_open(content.c_str(), user.c_str()) == BRIDGE_OK);

    const int id2 = user_create("待删除");
    REQUIRE(id2 > 1);
    REQUIRE(user_switch(id2) == BRIDGE_OK);
    ...
    REQUIRE(sqliteText(user, "SELECT COUNT(*) FROM profiles WHERE id = 2;") == "0");
    REQUIRE(sqliteText(user, "SELECT COUNT(*) FROM user WHERE id = 2;") == "0");
    REQUIRE(sqliteText(user, "SELECT COUNT(*) FROM reading_history WHERE user_id = 2;") == "0");
}

TEST_CASE("多用户 - 启动时清理历史软删除档案", "[multi_user]") {
    db_close();
    const std::string content = test_helpers::makeContentDb("mu_purge_old");
    const std::string user = test_helpers::makeUserDb("mu_purge_old");
    REQUIRE(db_open(content.c_str(), user.c_str()) == BRIDGE_OK);
    const int id2 = user_create("旧软删");
    REQUIRE(id2 > 1);
    db_close();

    // 模拟旧版本软删除留下的 deleted=1 行
    {
        sqlite3* db = nullptr;
        REQUIRE(sqlite3_open(user.c_str(), &db) == SQLITE_OK);
        const std::string sql = "UPDATE profiles SET deleted = 1 WHERE id = " + std::to_string(id2) + ";";
    ...
    REQUIRE(n == 1);
    REQUIRE(profiles[0].id == 1);
    REQUIRE(sqliteText(user, "SELECT COUNT(*) FROM profiles WHERE id = " + std::to_string(id2) + ";") == "0");
    db_close();
}
